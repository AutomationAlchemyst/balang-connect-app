...
import { getBlockedDates, blockDates, unblockDates } from '@/app/admin/actions';
...
export default function DateManager() {
    ...
    const handleBlockDates = async () => {
        ...
        try {
            ...
            const result = await blockDates(selectedDates, reason, token);
            if (result.success) {
            ...
            toast({ title: 'Error', description: error.message || 'Failed to block dates.', variant: 'destructive' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleUnblockDates = async () => {
        ...
        try {
            ...
            const result = await unblockDates(dateIdsToUnblock, token);
            if (result.success) {
                toast({ title: 'Success', description: result.message });
                setSelectedDates([]);
                await fetchDates();
            ...
            toast({ title: 'Error', description: error.message || 'Failed to unblock dates.', variant: 'destructive' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleUnblockSingleDate = async (dateId: string) => {
        ...
        try {
            ...
            const result = await unblockDates([dateId], token);
            if (result.success) {
                toast({ title: 'Success', description: result.message });
            ...
            toast({ title: 'Error', description: error.message || 'Failed to unblock date.', variant: 'destructive' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-12">
            <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-4">
                <div className="space-y-2">
                    <h2 className="text-3xl font-black uppercase text-[#041F1C] tracking-tighter">
                        Manage <span className="breezy-text-gradient italic">Blocked Dates</span>
                    </h2>
                    <p className="text-[#041F1C]/60 text-sm font-bold">
                        Set unavailability or closure dates for your events and catering booking.
                    </p>
                </div>

                <div className="breezy-glass-static p-4 flex items-start gap-3 max-w-sm shadow-md">
                    <div className="bg-[#0df2df]/20 p-2 rounded-xl text-[#09a093] shrink-0">
                        <Info className="h-4 w-4" strokeWidth={3} />
                    </div>
                    <div>
                        <h4 className="font-black uppercase text-[#041F1C] text-[10px] tracking-wider mb-0.5">Quick Guide</h4>
                        <p className="text-[#041F1C]/60 text-[10px] font-bold leading-normal">
                            Select dates on the calendar to block or unblock. Red dates are fully blocked.
                        </p>
                    </div>
                </div>
            </div>
            ...
            {fetchError && (
                <div className="bg-red-50 border-2 border-red-100 rounded-[2rem] p-8 flex items-center gap-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                <Card className="lg:col-span-7 breezy-glass-static border-0 overflow-hidden shadow-2xl">
                    ...
                    <CardContent className="p-8 flex flex-col md:flex-row gap-12 items-start">
                        <div className="p-6 bg-white/60 rounded-[2.5rem] border border-white/80 shadow-inner group w-full md:w-auto">
                            <Calendar
                                mode="multiple"
                                ...
                                classNames={{
                                    ...
                                    cell: "h-10 w-10 text-center text-sm p-0.5 relative",
                                    day: "h-9 w-9 p-0 font-bold text-[#041F1C]/70 hover:bg-[#0df2df]/20 hover:text-[#041F1C] hover:rounded-2xl transition-all duration-300",
                ...
                <Card className="lg:col-span-5 breezy-glass-static border-0 overflow-hidden shadow-2xl">
                    ...
                    <CardContent className="p-4">
                        {blockedDates.length > 0 ? (
                            <ul className="space-y-3 max-h-[400px] overflow-y-auto p-4 custom-scrollbar">
