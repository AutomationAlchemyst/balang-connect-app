...
export default function PaymentConfirmationDialog({
  ...
  const renderContent = () => {
    ...
    return (
      <div className="p-6 md:p-8 space-y-8 animate-in fade-in duration-300">
        {currentStep === 1 ? (
          <div key="payment-intro" className="space-y-6">
            <div className="p-6 bg-teal-50 border border-teal-100 rounded-2xl relative overflow-hidden group shadow-sm transition-all hover:shadow-md">
              ...
              <div className="space-y-3 font-bold text-slate-700 relative z-10 text-sm">
                <p className="flex justify-between items-center text-xs">
                  <span className="text-slate-400 font-black uppercase tracking-widest text-[9px]">Event Date</span>
                  <span className="bg-white px-3 py-1 rounded-lg border border-teal-100 shadow-sm">{format(eventConfig.eventDate, "PPP")}</span>
                </p>
                <div className="h-px bg-teal-100 w-full opacity-50 my-2"></div>
                <p className="flex justify-between items-center">
                  <span className="text-slate-400 font-black uppercase tracking-widest text-[9px]">Total Amount</span>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4 ml-1">
                ...
                <h3 className="text-slate-800 font-black uppercase text-xs tracking-widest">Select Payment Method</h3>
              </div>
                ...
                <div className="mt-4">
                  <TabsContent value="paynow_qr" className="p-6 bg-white border border-slate-100 rounded-2xl shadow-sm text-center">
                    ...
                    <div className="inline-block p-4 bg-white rounded-2xl shadow-lg border border-slate-50 relative z-10">
                      <Image src="/qr-code.png" alt="PayNow QR Code" width={180} height={180} className="mx-auto rounded-lg" />
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="fast_transfer" className="p-6 bg-white border border-slate-100 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <p className="text-center font-bold text-[9px] text-slate-400 uppercase tracking-widest">Recipient: Balang Kepalang SG</p>
                    </div>
                  </TabsContent>
                </div>
              </Tabs>
          ...
          <div key="payment-verification" className="space-y-8 py-2">
            <div>
              <div className="flex items-center gap-2 mb-4 ml-1">
                <div className="w-1 h-4 bg-teal-500 rounded-full"></div>
                <h3 className="text-slate-800 font-black uppercase text-xs tracking-widest">Upload Payment Proof</h3>
              </div>
              <div className="flex items-center justify-center w-full">
                <label htmlFor="paymentProof" className={cn(UPLOAD_ZONE_BASE, "h-48", uploadStatus === 'error' && "border-red-200 bg-red-50/50", uploadStatus === 'success' && "border-emerald-200 bg-emerald-50/30")}>
                  <div className="flex flex-col items-center justify-center py-4 text-center px-4 relative z-10">
                    {uploadStatus === 'idle' && <UploadCloud className="w-12 h-12 mb-3 text-teal-200 group-hover:text-teal-500 transition-all" strokeWidth={2} />}
                    {uploadStatus === 'reading' && <Loader2 className="w-12 h-12 mb-3 text-teal-500 animate-spin" strokeWidth={2.5} />}
                    {uploadStatus === 'success' && <CheckCircle className="w-14 h-14 mb-3 text-emerald-500 animate-in zoom-in duration-300" strokeWidth={2.5} />}
                    {uploadStatus === 'error' && <AlertCircle className="w-12 h-12 mb-3 text-red-400 animate-in shake duration-300" strokeWidth={2.5} />}

                    <p className="text-sm font-black text-slate-800 uppercase tracking-wider">
                      {uploadStatus === 'idle' && 'Attach Payment Receipt'}
                      {uploadStatus === 'reading' && 'Processing File...'}
                    ...
                    {uploadStatus !== 'error' && <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 bg-slate-100 px-3 py-1 rounded-full">JPG, PNG, PDF (Max 5MB)</p>}
                  </div>
            ...
            <div
              className={cn(
                "flex items-start space-x-4 p-6 rounded-[2rem] transition-all duration-300 cursor-pointer border-2 shadow-sm",
                consentedToTerms ? "bg-teal-50 border-teal-500/20" : "bg-slate-50 border-slate-200 hover:border-teal-300 hover:bg-white"
              )}
            ...
            >
              <div className="pt-0.5">
                <div className={cn(
                  ...
                  consentedToTerms ? "bg-teal-600 border-teal-600 text-white shadow-lg" : "border-slate-300 bg-white"
                )}>
              ...
              <div className="grid gap-1.5 leading-none">
                ...
                <p className="text-[11px] text-slate-500 font-bold uppercase leading-relaxed tracking-wider opacity-60">
  ...
  const renderFooter = () => {
    if (submissionStatus === 'success') {
      return (
        <DialogFooter className="p-8 sm:p-10 bg-teal-50 border-t border-teal-100 !justify-center flex-shrink-0">
          <Button variant="outline" onClick={() => handleDialogStateChange(false)} className={`${BUTTON_BASE} bg-white text-teal-700 border-teal-100 hover:bg-teal-50 h-14 px-10 w-full rounded-2xl`}>Return to Builder</Button>
        </DialogFooter>
      );
    }
    ...
    if (submissionStatus === 'error') {
      return (
        <DialogFooter className="p-8 sm:p-10 bg-red-50 border-t border-red-100 flex-col sm:flex-row gap-4 flex-shrink-0">
          <Button variant="ghost" onClick={() => handleDialogStateChange(false)} className={`${BUTTON_BASE} hover:bg-red-500 hover:text-white text-red-500 w-full h-14 rounded-2xl`}>Abort</Button>
          <Button onClick={() => setSubmissionStatus('idle')} className={`${BUTTON_BASE} bg-red-500 text-white hover:bg-red-600 w-full h-14 rounded-2xl`}>Retry</Button>
        </DialogFooter>
      );
    }

    return (
      <DialogFooter className="p-6 sm:p-8 bg-slate-50/50 backdrop-blur-md border-t border-slate-100 flex-col sm:flex-row gap-4 flex-shrink-0">
        <div className="flex justify-between items-center w-full gap-4">
          <div className="flex gap-2">
            <Button variant="ghost" onClick={currentStep === 1 ? onBack : () => setCurrentStep(1)} disabled={isActionDisabled} className="h-12 px-5 rounded-xl font-black uppercase text-slate-500 hover:text-slate-900 transition-all">
              {currentStep === 1 ? 'Back' : 'Back to Summary'}
            </Button>
            <Button variant="ghost" onClick={() => handleDialogStateChange(false)} disabled={isActionDisabled} className="h-12 px-5 rounded-xl font-black uppercase text-red-400 hover:bg-red-50 transition-all">
              Cancel
            </Button>
          </div>
          ...
          {currentStep === 1 ? (
            <Button
              ...
              className="bg-teal-600 hover:bg-teal-700 text-white h-14 px-10 font-display font-black uppercase tracking-wider rounded-2xl shadow-xl transition-all flex items-center gap-2 group"
            >
            ...
            <Button
              ...
              className="bg-gradient-to-r from-teal-600 to-emerald-500 hover:from-teal-500 hover:to-emerald-400 text-white h-14 px-10 font-display font-black uppercase tracking-wider rounded-2xl shadow-xl transition-all flex items-center gap-2"
        ...
        <DialogHeader className={HEADER_STYLE}>
          <div className="flex items-center justify-between relative z-10 w-full">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-white/20 backdrop-blur-md text-white flex items-center justify-center font-black text-lg shadow-lg border border-white/20">
                $
              </div>
              <div className="flex flex-col">
                <span className="text-white/70 font-black text-[10px] uppercase tracking-[0.2em] leading-none mb-1">Final Step</span>
                <DialogTitle className="text-white text-2xl font-display font-black uppercase tracking-tight leading-none">
                  Secure Payment
                </DialogTitle>
              </div>
            </div>
            <div className="flex flex-col items-end">
              <div className="flex gap-1.5 mb-1.5">
                <div className={cn("h-1.5 w-8 rounded-full transition-all", currentStep === 1 ? "bg-white w-12" : "bg-white/30")} />
                <div className={cn("h-1.5 w-8 rounded-full transition-all", currentStep === 2 ? "bg-white w-12" : "bg-white/30")} />
              </div>
              <span className="text-white/50 font-black text-[9px] uppercase tracking-widest">Step {currentStep} of 2</span>
            </div>
          </div>
          <DialogDescription className="text-teal-50/70 font-bold text-xs mt-4 relative z-10 max-w-sm flex items-center gap-2">
