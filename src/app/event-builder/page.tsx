...
export default function EventBuilderPage() {
  ...
  const handleFinalOrderSubmission = async (paymentProofUrl: string) => {
    if (!currentEventConfig || !customerDetailsForPayment || !currentEventConfig.eventDate) {
      toast({
        title: "Error: Missing Data",
        description: "Could not submit order because some details were missing. Please try again.",
        variant: "destructive",
      });
      return;
    }
    ...
    const finalOrderDetails = {
      customerName: customerDetailsForPayment.name,
      customerEmail: customerDetailsForPayment.email,
      customerPhone: customerDetailsForPayment.phone,
      eventDate: format(currentEventConfig.eventDate, "yyyy-MM-dd"),
      eventTime: currentEventConfig.eventTime || "",
      eventType: customerDetailsForPayment.eventType,
      venue: customerDetailsForPayment.venue,
      packageName: currentEventConfig.selectedPackage?.name || "Custom (Add-ons only)",
      pax: selectedPackage?.pax || "",
      flavors: currentEventConfig.selectedPackage?.flavors || [],
      addOns: currentEventConfig.addons.map(a => `${a.name} (x${a.quantity})`),
      total: currentEventConfig.totalPrice,
      paymentProofUrl: paymentProofUrl,
      status: 'Confirmed',
    };

    try {
      console.log("Saving order to Firestore...", finalOrderDetails);
      ...
      toast({
        ...
        description: "Your order has been saved. You will receive a WhatsApp confirmation shortly.",
      });
    ...
    <div className="space-y-8">
      ...
      {currentEventConfig && customerDetailsForPayment && currentEventConfig.eventDate && (
        <PaymentConfirmationDialog
          ...
          onConfirmOrder={handleFinalOrderSubmission}
        />
      )}
    </div>
  );
